...
import { emptyReceiptStockFilters, hasActiveReceiptStockFilters } from '../lib/receiptStockFilters'
import type { DocumentStatus, ReceiptStockFilterValues } from '../types'

const ALL = '__all__'

interface ReceiptStockFiltersBarProps {
  value: ReceiptStockFilterValues
  onChange: (value: ReceiptStockFilterValues) => void
}

/** Same filter contract as OpeningStockFiltersBar — Warehouse, Status, date range (the ticket's explicit filter list). */
export function ReceiptStockFiltersBar({ value, onChange }: ReceiptStockFiltersBarProps) {
  const warehouses = useQuery({ queryKey: ['warehouses-lookup'], queryFn: fetchWarehousesLookup })

  return (
    <FilterPanel onClear={() => onChange(emptyReceiptStockFilters)} hasActiveFilters={hasActiveReceiptStockFilters(value)}>
