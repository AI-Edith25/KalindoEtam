...
/**
 * "Cetak ringkasan tabel" — a merged printable table (one page, browser
 * "Save as PDF" produces the file), not the full per-document layout
 * SalesOrderBulkPrintPage renders. Same "ids override the filter" contract
 * as bulk export: `?ids=uuid,uuid,...` prints exactly those rows (reusing
 * the same fetchSalesOrder + React Query cache key as the single/bulk print
 * pages, so a document already viewed this session doesn't re-fetch);
 * otherwise every querystring param is forwarded to fetchSalesOrders as the
 * active filter, same list this was reached from. Mirrors
 * TandaTerimaInvoicePrintPage's merged-table approach.
 */
export function SalesOrderListPrintPage() {
  ...
  const byIdsQueries = useQueries({
    queries: ids.map((id) => ({ queryKey: ['sales-orders', id], queryFn: () => fetchSalesOrder(id), enabled: ids.length > 0 })),
  })

  const filterParams = Object.fromEntries(searchParams.entries())
  delete filterParams.ids
  const byFilterQuery = useQuery({
    queryKey: ['sales-orders', 'list-print', filterParams],
    queryFn: () => fetchSalesOrders({ page: 1, per_page: 100, ...filterParams }),
  ...
  if (isError) {
    return <ErrorState message="Failed to load Sales Orders for print." />
  }

  const rows: SalesOrder[] = ids.length > 0 ? byIdsQueries.map((query) => query.data!).filter(Boolean) : (byFilterQuery.data?.data ?? [])

  if (rows.length === 0) {
    return <ErrorState message="No Sales Orders match this selection." />
  }

  const companyName = brandingQuery.data?.name ?? 'PT. KALINDO ETAM'
  const total = rows.reduce((sum, row) => sum + Number(row.grand_total), 0)
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-4 bg-background p-6 text-foreground print:max-w-none print:p-[12mm]">
      ...
      <div className="flex items-start justify-between print:hidden">
        <h1 className="text-xl font-semibold">Sales Orders — List Print Preview ({rows.length})</h1>
      ...
      <div className="flex flex-col text-black text-[12px]" style={{ fontFamily: 'Arial, Helvetica, sans-serif' }}>
        ...
        <p className="mt-2 text-center text-base font-bold">SALES ORDERS LIST</p>
        ...
        <table className="mt-2 w-full border-collapse text-left">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id}>
                <td className="py-1 pr-2 align-top">{formatDate(row.order_date)}</td>
