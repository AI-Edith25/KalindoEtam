...
import { fetchDebitNote, fetchDebitNotes } from '../api/debitNoteApi'
...
/**
 * "Cetak ringkasan tabel" for Debit Notes — same recipe as
 * SalesOrderListPrintPage: `?ids=uuid,uuid,...` prints exactly those rows,
 * otherwise every querystring param forwards to fetchDebitNotes as the
 * active filter, same list this was reached from.
 */
export function DebitNoteListPrintPage() {
  ...
  const byIdsQueries = useQueries({
    queries: ids.map((id) => ({ queryKey: ['debit-notes', id], queryFn: () => fetchDebitNote(id), enabled: ids.length > 0 })),
  })

  const filterParams = Object.fromEntries(searchParams.entries())
  delete filterParams.ids
  const byFilterQuery = useQuery({
    queryKey: ['debit-notes', 'list-print', filterParams],
    queryFn: () => fetchDebitNotes({ page: 1, per_page: 100, ...filterParams }),
  ...
  if (isError) {
    return <ErrorState message="Failed to load Debit Notes for print." />
  }

  const rows: DebitNote[] = ids.length > 0 ? byIdsQueries.map((query) => query.data!).filter(Boolean) : (byFilterQuery.data?.data ?? [])

  if (rows.length === 0) {
    return <ErrorState message="No Debit Notes match this selection." />
  }
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-4 bg-background p-6 text-foreground print:max-w-none print:p-[12mm]">
      ...
      <div className="flex items-start justify-between print:hidden">
        <h1 className="text-xl font-semibold">Debit Notes — List Print Preview ({rows.length})</h1>
      ...
      <div className="flex flex-col text-black text-[12px]" style={{ fontFamily: 'Arial, Helvetica, sans-serif' }}>
        ...
        <p className="mt-2 text-center text-base font-bold">DEBIT NOTES LIST</p>

        <table className="mt-2 w-full border-collapse text-left">
          <thead>
            <tr className="border-b border-black">
              <th className="py-1 pr-2 font-bold">DATE</th>
              <th className="py-1 pr-2 font-bold">DEBIT NOTE NO</th>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id}>
                <td className="py-1 pr-2 align-top">{formatDate(row.debit_note_date)}</td>
                ...
                <td className="py-1 pr-2 align-top">{DEBIT_NOTE_REASON_LABELS[row.reason]}</td>
