...
import { fetchAccountsPayables } from '@/features/payment/api/accountsPayableApi'

/**
 * Read-only print view of the AP Detail Report's current filters — same
 * @media print + window.print() pattern as AccountsReceivableDetailReportPrintPage,
 * capped at the existing 100/page server limit (IndexAccountsPayableRequest, unchanged).
 */
export function AccountsPayableDetailReportPrintPage() {
  ...
  const listQuery = useQuery({
    queryKey: ['ap-detail-report-print', supplierId, warehouseId, status, agingBucket, dateFrom, dateTo, invoiceDateFrom, invoiceDateTo],
    queryFn: () =>
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-4 bg-background p-6 text-foreground print:max-w-none print:p-0">
      <div className="flex items-start justify-between print:hidden">
        <h1 className="text-xl font-semibold">AP Detail Report Print Preview</h1>
      ...
      {total > 100 && (
        <p className="rounded border border-amber-500/50 bg-amber-500/10 p-2 text-sm print:hidden">
          Showing 100 of {total} payables — narrow the filters to print everything.
        </p>
      )}
      ...
      <div className="border-2 border-foreground/80" style={{ fontSize: PRINT_FONT_SIZE_PX[printOptions.fontSize] }}>
        <div className="border-b-2 border-foreground/80 p-3">
          <h2 className="text-lg font-bold">ACCOUNTS PAYABLE DETAIL</h2>
          {(dateFrom || dateTo) && (
            <p>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="border-b border-foreground/30">
                <td className="border-r-2 border-foreground/80 p-2">{row.supplier?.supplier_name ?? '—'}</td>
          ...
          {listQuery.data?.meta && (
            <tfoot>
              <tr className="border-t-2 border-foreground/80 font-semibold">
                <td colSpan={8} className="border-r-2 border-foreground/80 p-2 text-right">
                  Total Outstanding
                </td>
